...
socket.on('newSrc', function(array){
	$('.medias-list ul li').each(function(i){
		// console.log(array[i]);
		$(this).attr('id', array[i]);
		$(this).find('source').attr('src', array[i]);
	});	
}); 
...
function init(){
    ...
    stop: function(event, ui) {
      ...
      socket.emit('newPosition', {array:newPositionsArr, currentFolder: currentFolder});
    }
});
	...
	$('body').on('click', '.close-audio',function(){
		...
		socket.emit('deleteEl', {pathEl:thisPath, id: thisId, currentFolder: currentFolder});
	});
